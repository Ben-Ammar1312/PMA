...
<head>
  <meta charset="utf-8">
  <title>${msg("executeActionsSubject")!msg("emailVerificationSubject")}</title>
  <style>
...
<body>
  <div class="email-container">
    ...
    <div class="email-content">
      <p>
        ${msg("executeActionsBodyIntroHtml",
              realmName,
              requiredActionsText?has_content?then(requiredActionsText, msg("noRequiredActions")))}
      </p>

      <p>
        <a href="${link}">${msg("executeActionsLinkText")!msg("emailVerifyLinkText")}</a>
      </p>

      <p style="font-size:14px; color:#6B7280;">
        ${msg("emailLinkExpirationHtml", linkExpirationFormatter(linkExpiration))}
      </p>

      <p style="font-size:14px; color:#6B7280;">
        ${msg("ignoreEmailIfNotRequested")}
      </p>
    </div>
